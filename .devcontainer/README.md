...
- `devcontainer.json` keeps the existing Ore contributor setup for working on this repository.
- `devcontainer.secure.json` adds a customer-oriented profile with stricter outbound network controls.

## Ore contributor profile

Use `devcontainer.json` when you are developing Ore itself. This is the same lightweight arm64 container that already exists in the repo.

## Secure customer profile

Use `devcontainer.secure.json` when you want a stricter runtime profile for running Ore inside a project container:

- installs the Ore CLI plus common build tools
- installs bubblewrap in setuid mode for Ore's Linux sandbox
- disables Docker's outer seccomp and AppArmor profiles so bubblewrap can construct Ore's inner sandbox
...
This profile keeps the stricter networking isolated to the customer path instead of changing the default Ore contributor container.
...
The secure profile's capability, seccomp, and AppArmor options are required when you want Ore's bubblewrap sandbox to run inside Docker as the non-root devcontainer user. Without them, Docker's default runtime profile can block bubblewrap's namespace setup before Ore's own seccomp filter is installed. This keeps the Docker relaxation explicit in the profile that is meant to run Ore inside a project container, while the default contributor profile stays lightweight.
