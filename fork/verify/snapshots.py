...
VERSION_TOKEN_RE = re.compile(
    r"\b\d+\.\d+\.\d+(-(?:alpha(?:\.\d+){0,2}|beta(?:\.\d+)?))?\b"
)
...
def run_help(bin_path: Path, args: list[str]) -> str | None:
    ...
    try:
        proc = subprocess.run(
            [str(bin_path), *args], capture_output=True, text=True, timeout=60, env=env
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--update", action="store_true", help="rewrite fixtures instead of comparing"
    )
    ...
    if not args.bin:
        print(
            "skip: no binary available yet — snapshots need a built entrypoint (--bin PATH)"
        )
        return 2
    ...
    if args.update:
        ...
        print(
            f"ok: {len(captures)} fixtures updated — review and commit the diff on delta"
        )
        return 0
    ...
    if not have_fixtures:
        print(
            "pending: no fixtures recorded yet — run snapshots.py --update --bin PATH once a "
    ...
    for rel, text in sorted(captures.items()):
        ...
        if not fixture.is_file():
            print(
                f"FAIL: fixture {fixture.relative_to(root)} missing (new subcommand?) — "
    ...
    stale = (
        [
            p
            for p in (snap_dir / "help").glob("*.txt")
            if p.relative_to(snap_dir).as_posix() not in captures
        ]
        if (snap_dir / "help").is_dir()
        else []
    )
    for p in stale:
        print(
            f"FAIL: stale fixture {p.relative_to(root)} — its subcommand no longer exists; "
            f"run snapshots.py --update"
        )
        fails += 1
