...
def load_manifest(here: Path) -> list[dict]:
    ...
    for c in checks:
        ...
        if c["suite"] not in SUITES:
            raise SystemExit(
                f"manifest.toml: check {cid}: suite {c['suite']!r} not in {SUITES}"
            )
        if c["severity"] not in SEVERITIES:
            raise SystemExit(
                f"manifest.toml: check {cid}: severity {c['severity']!r} not in {SEVERITIES}"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--artifact-dir",
        help="codex-package dir of shipped binaries (release by construction)",
    )
    ...
    for check in selected:
        ...
        if script is not None and not script.is_file():
            if pending_check:
                record(
                    "PENDING",
                    f"{check['script']} not present yet (lands with a later fork-verify commit)",
                )
            elif severity == "warn":
                ...
                record(
                    "FAIL",
                    f"{check['script']} missing — the suite is incomplete, not passing",
                )
            continue
        ...
        if check["release_required"] and use_bin and profile != "release":
            record(
                ...
                + (
                    ""
                    if profile == "debug"
                    else ", or pass --assume-release to vouch for it"
                ),
            )
            continue
        ...
        try:
            proc = subprocess.run(
                argv_check, capture_output=True, text=True, timeout=1800, cwd=cwd
            )
        ...
        if proc.returncode == STATUS_OK:
            ...
            record(
                "PENDING",
                f"{n} expected-not-yet-true finding(s) — see lines above, nothing failed",
            )
        else:
            reason = next(
                (
