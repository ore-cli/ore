#!/usr/bin/env bash
#
# VENDORED from upstream .github/scripts/macos-signing/
# notarize_macos_binary_with_rcodesign.sh at rust-v0.146.0, the last tag to
# carry it; byte-identical below this header (the legacy fork shipped
# ore-v0.146.2..5 through the same bytes). Upstream removed it in 0c07c7ee47
# "Use Azure Key Vault for macOS notarization (#37154)": the AKV replacement
# signs through a PKCS#11 provider and downloads its rcodesign build from a
# private az:// blob (.github/actions/setup-akv-pkcs11-codesigning/action.yaml)
# that a fork has no credentials to reach. Every piece of the rcodesign path is
# public — rcodesign itself installs from GitHub Releases via
# fork/release/install-rcodesign.sh — and the env contract is unchanged:
# APPLE_NOTARIZATION_KEY_P8 (base64 .p8) / APPLE_NOTARIZATION_KEY_ID /
# APPLE_NOTARIZATION_ISSUER_ID. If upstream reintroduces public notarize
# scripts, prefer theirs and delete this copy (sync checklist).
