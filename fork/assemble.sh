...
#   4. write-tree (the series branch never takes the generated passes); then
#      M = commit-tree TREE -p prev-main -p TAG^{commit}
...
# ------------------------------------------- 2b. record the base on the series
# delta has to name the tag it is actually rebased onto: lint-series.sh and this
# script both read fork/UPSTREAM at HEAD to find the base, and a stale one makes
# lint-series compute a base that is not an ancestor of HEAD and refuse to run
# at all.
#
# Pass 3g writes this same file again with assembled_at and series_head filled
# in. That copy is main's, and the static suite keys on it: tree_is_assembled()
# is `bool(assembled_at)`, so a non-empty value flips every check into
# on-main-proof mode. This copy is delta's, so both fields stay empty -- the
# shape delta has always carried.
#
...
series_head = ""
EOF
wt add fork/UPSTREAM
_tip_slug=$(wt log -1 --format='%(trailers:key=Fork-Patch,valueonly)' | tr -d '[:space:]')
if wt diff --cached --quiet HEAD; then
  info "series already records $TAG as its base"
elif [[ "$_tip_slug" == "$BASE_RECORD_SLUG" ]]; then
  wt -c "user.name=$BOT_NAME" -c "user.email=$BOT_EMAIL" -c commit.gpgsign=false \
    commit --quiet --amend --no-edit
  info "series base record amended to $TAG"
else
  wt -c "user.name=$BOT_NAME" -c "user.email=$BOT_EMAIL" -c commit.gpgsign=false \
    commit --quiet -F - <<EOF
fork: record $TAG as the series base

Written by fork/assemble.sh after the rebase. delta must name the tag its
commits are actually sitting on, because lint-series.sh and assemble.sh both
resolve the base by reading this file at HEAD.

assembled_at and series_head are deliberately empty here: they are what
tree_is_assembled() reads to tell main from delta, and only the generated pass
on main fills them in.

Fork-Patch: $BASE_RECORD_SLUG
Purpose: Keep delta's recorded base honest across a rebase.
Invariant: fork/UPSTREAM at delta names the tag the series is rebased onto, with assembled_at empty.
Verify: fork/lint-series.sh runs without a base error; fork/verify/version_check.py reports scheme C against this tag.
EOF
...
# --------------------------------------------------- 3h. the assembled tree
#
# write-tree, NOT a commit on sync-delta/$TAG. The generated passes are main's
# content -- the substitution pass above all -- and sync-delta is the ref
# finalize advances delta to, so committing them here is how they reached delta
# the first time a sync landed. A substituted delta then makes the NEXT assemble
# apply the rebrand rules to text that is already rebranded: twelve of them
# match nothing and the stale-rule check fails the run. The series branch stays
# the series, which is also what the "series sync-delta/$TAG = $SERIES_HEAD"
# line below has always claimed it was.
#
# The passes are not lost: they are in $PASSES_LOG, and with WORKTREE_KEEP=1 the
# worktree keeps HEAD at the series with the assembled files in place, so
# `git diff HEAD` there is exactly what the passes did.
wt add -A
TREE=$(wt write-tree)
if [[ "$TREE" == "$(wt rev-parse 'HEAD^{tree}')" ]]; then
  info "generated passes produced no changes"
fi
