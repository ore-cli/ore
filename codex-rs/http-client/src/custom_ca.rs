//! Custom CA handling shared by Ore outbound HTTP and websocket clients.
//!
//! Ore constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!   because Ore itself commonly runs spawned test processes under seatbelt, so this is not just
...
/// Builds a reqwest client that honors Ore custom CA environment variables.
///
...
/// easy mistake to make when adding a new outbound Ore HTTP path, and the resulting bug only
...
/// Builds a rustls client config using native roots and any configured Ore custom CA bundle.
///
...
fn build_rustls_client_config(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Ore custom
...
trait EnvSource {
    ...
    ///
    /// `CODEX_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Ore-specific override.
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Ore CA overrides.
    ///
    /// This accepts a few common real-world variants so Ore behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Ore only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
