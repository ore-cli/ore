//! Validates and pins local Ore home and sandbox directories, keeping sandbox
...
impl std::fmt::Display for UnsupportedHomeDrive {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("Ore home must be located on a fixed local drive")
    }
}
...
pub(super) fn prepare_codex_home(requested: &Path) -> Result<(PathBuf, Vec<OwnedHandle>)> {
    ...
    let requested_root = requested
        .ancestors()
        .last()
        .context("find the root of the requested Ore home")?;
    ...
    let parent = requested
        .parent()
        .context("Ore home must have an existing parent directory")?;
    ...
    let home = requested
        .canonicalize()
        .with_context(|| format!("canonicalize Ore home {}", requested.display()))?;
    ...
    .enumerate()
    {
        ...
        let handle = pin_directory(directory, access, DirectoryOpenDisposition::OpenOrCreate)
            ...
            if index == 0 {
                format!(
                    "requesting user must be permitted to write Ore home {}",
                    directory.display()
                )
            } else {
                format!(
