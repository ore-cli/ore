...
#[command(
    version,
    override_usage = "ore exec [OPTIONS] [PROMPT]\n       ore exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Ore outside a Git repository.
...
struct ResumeArgsRaw {
    ...
    // so "ore resume --last <prompt>" treats the positional as a prompt, not a session id.
