...
//! the configured Ore executable as a subprocess, accepts only valid JSON from
//! `ore doctor --json --feedback`, derives a small set of Sentry tags, and otherwise
...
/// Runs `ore --cd <workspace> doctor --json --feedback` and returns a best-effort
/// feedback attachment.
///
/// Failure to spawn Ore, finish before the timeout, or parse JSON means the
