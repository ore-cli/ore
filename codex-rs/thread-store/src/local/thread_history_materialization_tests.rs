...
/// Separate Ore and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let runtime = codex_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from Ore home");
    assert!(
        runtime
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            ...
            "SQLite database should not be created under Ore home"
        );
    }
}
...
async fn paginated_fork_reads_compressed_shared_lineage_without_materializing() {
    ...
    assert!(matches!(
        error,
        crate::ThreadStoreError::InvalidRequest { message } if message.contains("must be in Ore home")
    ));
    store
        .shutdown_thread(source_thread_id)
        .await
