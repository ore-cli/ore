...
fn resolve_windows_update_command_from_path(
    ...
    if path_env.is_empty() {
        anyhow::bail!(
            "Could not find an absolute update command `{command}` on PATH. Please update manually: https://github.com/ore-cli/ore#install"
        );
    }
        ...
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!("`ore agents` does not accept an initial prompt or images");
                }
                ...
                {
                    anyhow::bail!(
                        "`ore agents` cannot apply local provider or additional-directory overrides to a remote server"
                    );
                }
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `ore {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `ore {subcommand}`"
        );
    }
    Ok(())
}
...
fn reject_unsupported_worktree_for_subcommand(
    ...
    match subcommand {
        ...
        Some(Subcommand::Fork(_)) => {
            anyhow::bail!("`ore fork --worktree` requires an explicit session ID")
        }
        Some(Subcommand::Exec(command)) => match &command.command {
            None | Some(ExecCommand::Fork(_)) => Ok(()),
            Some(ExecCommand::Resume(_)) => anyhow::bail!(
                "`--worktree` cannot resume an existing session; use `ore exec fork --worktree`"
            ),
        ...
        _ => {
            anyhow::bail!(
                "`--worktree` supports new interactive sessions, `ore fork`, `ore exec`, and `ore exec fork`"
            )
        }
    }
}
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `ore {subcommand}`");
    }
    Ok(())
}
...
mod tests {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let codex_home = find_codex_home().expect("ore home");
        codex_app_server::app_server_control_socket_path(&codex_home)
    ...
    async fn format_exit_messages_preserves_auth_env_through_tui_runner() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "Reconnect: ore --remote wss://example.com:443/ --remote-auth-token-env CODEX_REMOTE_TOKEN resume 123e4567-e89b-12d3-a456-426614174000",
                "Stop the current turn: run ore --remote wss://example.com:443/ --remote-auth-token-env CODEX_REMOTE_TOKEN agents, select this task, and press ctrl + x.",
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "  ore resume 123e4567-e89b-12d3-a456-426614174000".to_string(),
            ]
        );
    }
        ...
        insta::allow_duplicates! {
            for thread_name in [None, Some("")] {
                ...
                To continue this session, run:
                  ore resume 123e4567-e89b-12d3-a456-426614174000
                ");
            }
        }
    }
    ...
    fn format_exit_messages_applies_color_when_enabled() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2",
                "To continue this session, run:",
                "  \u{1b}[36more resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        To continue this session, run:
          ore resume 123e4567-e89b-12d3-a456-426614174000
        Or run ore resume and select my-thread.
        ");
    }
    ...
    fn format_exit_messages_colors_commands_and_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run:",
                "  \u{1b}[36more resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
                "Or run \u{1b}[36more resume\u{1b}[39m and select \u{1b}[36mmy-thread\u{1b}[39m.",
            ]
        );
    }
