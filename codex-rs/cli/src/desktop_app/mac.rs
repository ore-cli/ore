...
mod tests {
    ...
    async fn rejects_valid_signature_without_openai_signing_identity() {
        ...
        let app_path = temp_dir.path().join("Codex.app");
        let contents_path = app_path.join("Contents");
        let executable_path = contents_path.join("MacOS/Ore");
        ...
        fs::write(
            contents_path.join("Info.plist"),
            r#"<?xml version="1.0"?><plist version="1.0"><dict><key>CFBundleIdentifier</key><string>com.openai.codex</string><key>CFBundleExecutable</key><string>Ore</string></dict></plist>"#,
        )
        .expect("write Info.plist");
