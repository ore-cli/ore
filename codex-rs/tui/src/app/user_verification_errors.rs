...
pub(super) fn verification_error_message(error: &TypedRequestError) -> &'static str {
    let TypedRequestError::Server { source, .. } = error else {
        return "Could not complete user verification with the local Ore binary.";
    };
    let details = source
        .data
        .clone()
    ...
    match details {
        ...
        }) => "The local Ore binary could not verify this request.",
        Some(UserVerificationErrorDetails::Unavailable {
            reason: UserVerificationUnavailableReason::CredentialMissing,
        }) => "No user-verification credential is available in the local Ore binary.",
        Some(UserVerificationErrorDetails::Unavailable {
        ...
        | None => "The local Ore binary could not complete user verification.",
    }
}
