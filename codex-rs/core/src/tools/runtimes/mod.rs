...
pub(crate) fn exec_env_for_sandbox_permissions(
    ...
    // defensive cleanup for a caller that passes an environment already prepared by Ore.
...
/// PATH entries owned by Ore runtime setup.
///
...
///
/// `runtime_path_prepends` contains Ore-owned PATH entries already applied to
