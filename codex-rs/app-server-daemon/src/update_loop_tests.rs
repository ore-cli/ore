...
fn manual_update_daemon(home: &TempDir) -> (Daemon, String) {
    ...
    let bin = standalone.join("releases").join(&release).join("ore");
    ...
    (
        Daemon {
            ...
            managed_codex_bin: standalone.join("current/ore"),
        },
        release,
    )
}
...
async fn manual_update_restarts_managed_daemon_with_automatic_updates_disabled() {
    ...
    assert_eq!(
        ...
        standalone.join("current/bin/ore")
    );
