...
#[cfg(windows)]
const INSTALL_URL: &str = "https://github.com/ore-cli/ore/releases/latest/download/install.ps1";
...
fn selected_release(daemon: &Daemon) -> Result<(&Path, std::path::PathBuf, String)> {
    let home = daemon
        ...
        .context("daemon settings path has no Ore home")?;
...
async fn run_installer_script(
    ...
    let status = status.context("failed to wait for standalone Ore updater")?;
